...
<%-- <%
	...
	if(sid == null) {
		out.write("<script>alert('정상적인 접근이 아닙니다. 로그인을 먼저 진행해주세요.');");
		out.write("location.href=('http://localhost:9000/login/login.jsp');</script>");
	} else {
		
	if(!sid.equals("admin")){
		out.write("<script>");
		out.write("alert('관리자 접근 권한이 필요합니다. 다시 로그인 해주세요.');");
...
<body>
	...
	<div class="content">
		<section class="admin">
			...
			<div>
				<article><a href="/admin/notice_list/1">🛕 공지사항 관리</a></article>
				<article>🚗영화 관리</article>
				<article><a href="/admin/member_list/1">👩회원 관리</a></article>
			</div>
		</section>
	</div>
